...
;(function($, window, undefined) {
        ...
        if (this._parentHeight - this._offset > height && !this._frozen) {
            if (scrollTop >= this._start && scrollTop <= realStop) {
                if(this.options.useFixed){
                    this.element.css({'position':'fixed','top':this.options.topOffset,'left':this.element.offset().left, 'width': this.element.outerWidth()});
                } else {
                    this.updateOffset(scrollTop - this._start);
                }
            } else {
                this.element.css({'position':'relative','left':0, 'width': ''});
    ...
    Sticky.prototype.setBoundaries = function() {
        ...
        this._start = this.parent.offset().top + this._offset - this.options.topOffset;
